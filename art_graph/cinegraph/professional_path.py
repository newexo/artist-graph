...
def professional_subgraph(g: nx.Graph, jobs):
    """
    Get edge subgraph containing where work nodes are connected only by collaborators
    with particular jobs. For example, if the professional graph is one where works are
    movies and collaborators are actors, directors and writers, this function can be
    used to find the subgraph connected by actors.
